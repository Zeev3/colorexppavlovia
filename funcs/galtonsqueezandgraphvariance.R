...
scale_df <- expand.grid(
  ...
  variance_level = levels(filt_dat$variance_level),
  tick = tick_vals
)

scale_df$condition <- factor(scale_df$condition, levels = levels(filt_dat$condition))
scale_df$variance_level <- factor(scale_df$variance_level, levels = levels(filt_dat$variance_level))
...
reverse_galton_data <- filt_dat %>%
  group_by(indexSelected, variance_level, condition) %>%
  summarise(
...
ggplot(reverse_galton_data) +
  ...
  labs(
    title = "Reverse Galton squeeze diagrams",
    x = NULL,
    y = NULL
  ) +
  theme_classic(base_size = 14) +
...
ggplot(reverse_galton_data_avg) +
  ...
  facet_wrap(~ variance_level, nrow = 1) +
  scale_x_continuous(
    breaks = c(1, 2),
    labels = c("Mean response", "Objective mean"),
...
scatter_bucketed <- filt_dat %>%
  ...
  group_by(response_bucket, bucket_label, variance_level, condition) %>%
  summarise(
    mean_true = mean(meanVal, na.rm = TRUE),
    n = n(),
    .groups = "drop"
...
ggplot(scatter_bucketed, aes(x = response_bucket, y = mean_true, color = variance_level)) +
  geom_line(aes(group = variance_level), linewidth = 0.8, alpha = 0.7) +
  ...
  labs(
    ...
    color = "Variance level"
  ) +
  theme_classic(base_size = 14)

# Average only
ggplot(scatter_avg, aes(x = response_bucket, y = mean_true, color = variance_level)) +
  geom_line(aes(group = variance_level), linewidth = 0.9, alpha = 0.7) +
...
# Experience only
ggplot(scatter_exp, aes(x = response_bucket, y = mean_true, color = variance_level)) +
  geom_line(aes(group = variance_level), linewidth = 0.9, alpha = 0.7) +
  geom_point(size = 3, alpha = 0.9) +
  geom_abline(
    slope = 1, intercept = 0,
...
scatter_forward <- filt_dat %>%
  ...
  group_by(mean_bucket, variance_level, condition) %>%
  summarise(
    mean_response = mean(indexSelected, na.rm = TRUE),
    n = n(),
    .groups = "drop"
...
# Average only
ggplot(scatter_forward_avg, aes(x = mean_bucket, y = mean_response, color = variance_level)) +
  geom_line(aes(group = variance_level), linewidth = 0.9, alpha = 0.7) +
  geom_point(size = 3, alpha = 0.9) +
  geom_abline(
    slope = 1, intercept = 0,
...
# Experience only
ggplot(scatter_forward_exp, aes(x = mean_bucket, y = mean_response, color = variance_level)) +
  geom_line(aes(group = variance_level), linewidth = 0.9, alpha = 0.7) +
  geom_point(size = 3, alpha = 0.9) +
  geom_abline(
    slope = 1, intercept = 0,
