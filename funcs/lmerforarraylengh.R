...
setwd("/Users/zeevbenamos/Documents/GitHub/colorexppavlovia/data/greaternoise/colorsquersexpgreaternoise") 
...
contrasts(filt_dat$array_length )
...
RttM_model_main <- lmer(
  indexSelected ~ meanVal_c * array_length  +
...
emtrends(RttM_model_main, ~ array_length , var = "meanVal_c")
pairs(emtrends(RttM_model_main, ~ array_length , var = "meanVal_c"))

# -----------------------------
# Predictions for plotting
# -----------------------------
predictions_across <- ggpredict(RttM_model_main, terms = c("meanVal_c [all]", "array_length"))
...
ggplot(predictions_across, aes(x = x, y = predicted, color = group, fill = group)) +
  ...
  labs(
    ...
    title = "Average estimation as a function of objective mean and number of squares presented ",
